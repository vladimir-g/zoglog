...
- log-server (&key server port nick channels username password extra-commands encoding socket-timeout tls) ::
     start logging for one server in current thread. Default encoding
     is UTF-8. If ~tls~ is provided, SSL/TLS connection will be
     used. If ~username~ and ~password~ are provided, SASL
     authentication will be used. Example:
